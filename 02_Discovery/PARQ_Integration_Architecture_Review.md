...
| INT-04 | Account lifecycle: delete/reactivate/password | Mobile App / IAM | SSO, FS, Buzzee Bee | Synchronize account state and entitlements across systems | Account status, deletion date, reactivation request, password state | Real-time + scheduled hard delete | High | Proposal states 30-day soft delete and hard delete on Day 31, but does not define which system enforces the lifecycle clock. |
...
| P1 | Account delete, soft delete, reactivation, hard delete | Mobile App, IAM, SSO, FS, Buzzee Bee | Cross-system lifecycle timing is risky and PDPA-sensitive. |
...
8. Who owns the 30-day soft delete clock and Day 31 hard delete execution?
