...
| LIB-010 | Implement approved Portal v2 Project Hub design. | Libra | Done with confirmation item | Transformed `07_Portal/` top navigation to Home, Project, Discovery, Architecture, Delivery, QA, and Work by Role. Added unified Project Controls, role-based entry points, Project Command Center homepage, diagram context metadata, and v2 data files. Current Gate wording requires Bas confirmation in `CONF-V2-001`. |
...
| LIB-012 | Implement Portal v2.1 homepage, metadata, discovery, and Mermaid observability fixes. | Libra | Done with confirmation item | Improved Home mission-control scanability, compacted metadata on Home/Project/Discovery, added Timeline/Roadmap placeholders, fixed Discovery Business Rules routing, prevented PARQ User Flow XLSX download routing, added Google Sheet source confirmation request, and added Mermaid validation metadata/error visibility. |
