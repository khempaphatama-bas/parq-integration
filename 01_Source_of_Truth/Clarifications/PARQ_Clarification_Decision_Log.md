...
| DEC-REQ-003 | Feature Breakdown Matrix v0.4 shall separate Tower Context Management as E10 and update traceability mapping. Traffic Monitoring routes to existing PARQ/OBK traffic source only. QR Identity, Tower Context, and Physical Access shall be separated for estimation boundaries. Parking feature label remains `Argento Payment Integration`. Profile Management keeps `Edit Profile` with PARQ workplace metadata view-only from FS. CMS keeps `PARQ User Visibility` with Phase 1 view-only scope note. Account Lifecycle shall be split under E1 into company offboarding / Workplace persona removal, account deletion, reactivation within 30 days, and Day 31 hard delete. | Bas clarification on 2026-06-16 during Feature Breakdown Matrix revision. | Decision recorded / applied in `PARQ-REQ-003` and `PARQ-REQ-002` v0.2 | Molly, Simon, Libra, Developer estimation team, Quinn later |
...
| SF-REQ-002 | `PARQ-REQ-003` Feature Breakdown Matrix v0.4 is a product-view estimation artifact, not a user-story, acceptance-criteria, QA, API-contract, or technical-design artifact. It includes a Jira import CSV companion file with Epic rows and feature Task rows. | Bas clarification on 2026-06-16; `02_Discovery/PARQ_Feature_Breakdown_Matrix.md`; `02_Discovery/PARQ_Feature_Breakdown_Matrix_Jira_Import.csv`. | Draft / Ready for Bas Review |
...
| 2026-06-16 | Molly | Recorded Bas Feature Breakdown Matrix v0.4 clarifications and applied them to `PARQ-REQ-003` plus `PARQ-REQ-002` Tower Context traceability mapping. Added Jira import CSV companion file for `PARQ-REQ-003`. | Complete; ready for Bas review |
